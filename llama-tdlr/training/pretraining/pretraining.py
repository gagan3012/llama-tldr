...
def main():
    ...
    with training_args.main_process_first(desc="dataset map tokenization and grouping"):
        ...
        files = glob(data_args.dataset_dir+"/*.txt")
        if training_args.debug_mode is True:
            files = [files[0]]
        for idx, file in enumerate(files):
            data_file = os.path.join(path, file)
            filename = file.replace(".txt", "")
            cache_path = os.path.join(data_args.data_cache_dir, filename, "text_large_v3")
            os.makedirs(cache_path, exist_ok=True)
            try:
                processed_dataset = datasets.load_from_disk(cache_path, keep_in_memory=False)
                logger.info(f'training datasets-{filename} has been loaded from disk')
            except Exception:
                cache_dir = os.path.join(data_args.data_cache_dir, filename+"_text_large_v3")
